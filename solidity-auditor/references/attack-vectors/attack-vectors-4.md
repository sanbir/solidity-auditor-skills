...
- **D:** EIP-2535 Diamond facets declare top-level state variables instead of namespaced storage structs. Multiple facets start at slot 0, corrupting each other.
- **FP:** All facets use single `DiamondStorage` struct at namespaced position (EIP-7201). OZ `@custom:storage-location` pattern.
